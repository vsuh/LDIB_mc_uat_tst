...
Функция ПрочитатьПараметры(Отказ)
	Если АргументыКоманднойСтроки.Количество() = 0 Тогда
		ВызватьИсключение "Требуется параметр в строке вызова";
	Иначе
	...
	Если ПустаяСтрока("" + ИмяАдминистратораИБ + ПарольАдминистратора) Тогда
		ВызватьИсключение "Не определены переменные среды ""ibADMIN"" или ""ibPASSWORD""";
	КонецЕсли;
...
Функция ПолучитьПараметрыИБ(ИмяИБ, Отказ)
	Если ПустаяСтрока(имяФайлаПодрядчиков) Тогда
		ВызватьИсключение "Не определено имя файла параметров";
	КонецЕсли;
	ФайлПроверки = Новый Файл(имяФайлаПодрядчиков);
	Если Не ФайлПроверки.Существует() Тогда
		ВызватьИсключение "Не найден файл параметров " + ФайлПроверки.ПолноеИмя;
	КонецЕсли;
	Если Отказ Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Исключение
		ЧтениеДж.Закрыть();
		ВызватьИсключение "Ошибка десериализации данных из файла " + имяФайлаПодрядчиков;
	КонецПопытки;
...
Процедура ПредоставитьПолныеПрава(СтруктураПараметров, Отказ)
	...
	Если СтруктураПараметров = Неопределено Тогда
		ВызватьИсключение "Параметры не получены";
	КонецЕсли;
