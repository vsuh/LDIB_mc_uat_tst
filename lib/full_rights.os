...
Функция ПрочитатьПараметры(Отказ)
	...
	// ЗаписьТекста = новый ЗаписьТекста("PWD.PWD");
	// ЗаписьТекста.ЗаписатьСтроку("%%%" + ИмяАдминистратораИБ + " : " + ПарольАдминистратора + "%%%");
...
Процедура ПредоставитьПолныеПрава(СтруктураПараметров, Отказ)
	...
	КонецПопытки;
	Если (НРег(Лев(СтруктураПараметров.ИБ, 6)) = "mc_zup") Тогда
		Попытка
